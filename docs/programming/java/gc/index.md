...
Full GC 触发（特有场景）：
   - 混合回收（Mixed GC）无法释放足够空间：G1 通过混合回收逐步回收老年代分区，若混合回收后老年代仍无足够空间，触发 Full GC；
   - 晋升失败（Evacuation Failure）：新生代对象晋升时，目标分区（老年代 / 幸存区）空间不足，触发 Full GC；
   - 并发标记周期超时：G1 的并发标记未在指定时间内完成，触发 Full GC；
   - 元空间不足、显式调用System.gc()。
   - 执行逻辑：G1 的 Full GC 是退化的单线程收集器（类似 Serial Old），STW 时间长；为避免 Full GC，G1 会通过调整混合回收的分区数、并发标记周期等参数优化，生产环境需重点监控 G1 的混合回收效率。
#### ZGC/Shenandoah（低延迟收集器）

Full GC 触发：这类收集器几乎不会触发传统 Full GC，其设计目标是 “无停顿的并发回收”，仅在极端场景下（如堆内存被耗尽、元空间溢出且无法卸载类）才会触发 Full GC；
   - 执行逻辑：ZGC/Shenandoah 通过并发标记 - 重定位 - 重映射实现全堆回收，即使触发 Full GC，也会通过并发机制减少 STW 时间（通常毫秒级），远优于传统收集器