...
- **去噪与清洗**：
    - 剔除明显噪声：广告、乱码、无关重复内容、模板化文本，模型语义去重
...
| **LLM 辅助分块（FastGPT项目支持）** | 提示 LLM 将长文本划分为“语义独立、自包含”的块 | 高价值文档，可接受较高成本 |
...
## 9. RAG 核心评估指标优化指南
RAGAS 是一个专门用于 评估 RAG（检索增强生成）系统质量 的开源评估框架。它不依赖人工标注，而是通过 无参考（reference-free）的自动化指标，从多个维度衡量 RAG 系统的性能。

RAGAS 利用 另一个大语言模型（如 GPT-4、Qwen-Max）作为“评判员”，通过精心设计的 prompt 让 LLM 打分：

RAGAS 既支持无监督（无需标准答案）评估，也支持有监督（提供 ground truth）评估，灵活适配不同数据条件
